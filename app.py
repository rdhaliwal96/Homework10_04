...
def preciptation():
    ...
    """
    results = session.query(Measurements.date, Measurements.prcp).filter(Measurements.date >= '2016-08-01', Measurements.date <= '2017-08-31').all()
...
def temps(start, end='2017-08-23'):
    ...
    def calc_temps(start_date, end_date):
        results = session.query(Measurements.tobs).filter(Measurements.date >= start_date, Measurements.date <= end_date).all()
